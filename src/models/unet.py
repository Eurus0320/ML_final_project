...
class UNet():
    ...
    def load_data(self):
        ...
        self.imgs_trn, self.msks_trn = isbi_get_data_montage('dataset/train_set/train_img', 'dataset/train_set/train_label',
                                                             nb_rows=15, nb_cols=5, rng=np.random)
        self.imgs_val, self.msks_val = isbi_get_data_montage('dataset/train_set/train_img', 'dataset/train_set/train_label',
                                                             nb_rows=5, nb_cols=15, rng=np.random)
        return
...
def main():
    ...
    subprs_sbt.add_argument('-t', '--tiff', help='path to tiffs', default='dataset/test_set/test_img/0.tif')
    ...
    if args['which'] == 'train':
        ...
        model.train()
        print("finish")
        model.net.save_weights('./weights.hdf5')


    elif args['which'] == 'submit':
        out_path = '%s/0.tif' % model.checkpoint_path
        ...
        imgs_sbt = imgs_sbt.transpose(2,0,1)
